...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--archive", type=Path, default=None, help="Already downloaded VisDrone-VID archive.")
...
def main() -> None:
    ...
    archive_path = args.archive or args.output_root / "archives" / split["archive"]
    if args.archive is None:
        ...
        raise FileNotFoundError(f"VisDrone archive does not exist: {archive_path}")
    if args.extract:
    ...
    write_manifest(
        args.output_root / f"phase1_3_visdrone_vid_{args.split}_manifest.json",
        {
            ...
            "expected_config": (
                "configs/datasets/visdrone/visdrone_vid_val_uav0000086.yaml"
                if args.split == "val"
                else f"configs/datasets/visdrone/visdrone_vid_{args.split}.yaml"
            ),

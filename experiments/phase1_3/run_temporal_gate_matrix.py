"""Run Phase 1.3-B temporal gate POC across dataset experiment configs."""
...
DEFAULT_MATRIX_OUTPUT_ROOT = "outputs/temporal_gate_matrices"


@dataclass(frozen=True)
class TemporalMatrixExperiment:
    name: str
    dataset: Path
    limit: int
...
def load_temporal_matrix_experiment(path: str | Path) -> TemporalMatrixExperiment:
    ...
    return TemporalMatrixExperiment(
...
def build_temporal_matrix_plan(
    experiment_configs: list[str | Path],
    run_output_root: str | Path,
    run_suffix: str,
) -> list[PlannedTemporalRun]:
    plan: list[PlannedTemporalRun] = []
    for experiment_config in experiment_configs:
        experiment = load_temporal_matrix_experiment(experiment_config)
        run_id = f"{experiment.name}_temporal_gate_{run_suffix}"
        run_root = Path(run_output_root) / run_id
        command = (
            sys.executable,
            str(PROJECT_ROOT / "experiments" / "phase1_3" / "run_temporal_gate_poc.py"),
            "--dataset-config",
...
def execute_temporal_matrix(
    ...
    manifest = {
        "schema_version": 1,
        "pipeline_type": "temporal_gate_matrix",
    ...
    if not dry_run:
        for index, planned in enumerate(plan):
            ...
            write_json(manifest, matrix_root / "manifest.json")
        summary = summarize_temporal_runs(plan)
        write_json(summary, matrix_root / "summary.json")
        write_text(render_temporal_summary_markdown(summary), matrix_root / "summary.md")
    return manifest


def summarize_temporal_runs(plan: list[PlannedTemporalRun]) -> dict[str, Any]:
    runs = []
    for planned in plan:
        report_path = planned.run_root / "reports" / "temporal_gate_report.json"
        ...
        runs.append(
            {
                ...
                "scene_delta_mean_average": data["scene_delta_mean_average"],
...
def render_temporal_summary_markdown(summary: dict[str, Any]) -> str:
    lines = [
        "# Temporal Gate Matrix Summary",
        "",
        (
            "| Dataset | Profile | Detector call reduction | Target-frame recall | "
    ...
    for run in summary["runs"]:
        for profile_name, profile in run["profiles"].items():
            lines.append(
                ...
                f"`{profile_name}` | "
                f"{_format_ratio(profile['detector_call_reduction'])} | "
                f"{_format_ratio(profile['target_frame_recall'])} | "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Phase 1.3-B temporal gate matrix.")
...
def main() -> None:
    ...
    plan = build_temporal_matrix_plan(args.experiment_config, args.run_output_root, suffix)
    matrix_root = Path(args.matrix_output_root) / f"phase1_3_temporal_gate_{suffix}"
    manifest = execute_temporal_matrix(
        plan=plan,

"""Run Phase 1.3-D feedback / tracker-memory POC on one annotated dataset."""
...
from evaluation.reports.tracker_memory import (
    ...
    default_tracker_memory_profiles,
    write_tracker_memory_report_json,
    write_tracker_memory_report_markdown,
)

DEFAULT_OUTPUT_ROOT = "outputs/tracker_memory_poc"


def run_tracker_memory_poc(
    ...
    selected_run_id = run_id or f"{name}_tracker_memory_{suffix}"
    run_root = Path(output_root) / selected_run_id
    report_root = run_root / "reports"
    frame_records_path = run_root / "tracker_memory_frames.jsonl"
    report_json_path = report_root / "tracker_memory_report.json"
    report_md_path = report_root / "tracker_memory_report.md"

    report, frame_records = build_tracker_memory_report(
        frames=frames,
        ground_truth=annotations,
        profiles=default_tracker_memory_profiles(),
        dataset_config=project_relative(dataset_path),
        experiment_name=name,
        target_classes=target_classes,
    )
    write_jsonl(frame_records, frame_records_path)
    write_tracker_memory_report_json(report, report_json_path)
    write_tracker_memory_report_markdown(report, report_md_path)

    manifest = {
        "schema_version": 1,
        "pipeline_type": "tracker_memory_poc",
        "experiment_name": name,
        ...
        "summary": {
            ...
            "profiles": {
                name: {
                    ...
                    "average_memory_roi_count_per_memory_frame": (
                        profile.average_memory_roi_count_per_memory_frame
                    ),
...
def _load_frame_metadata(dataset_config) -> list[TrackerMemoryFrame]:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Phase 1.3-D tracker memory POC.")
...
def main() -> None:
    args = parse_args()
    manifest = run_tracker_memory_poc(

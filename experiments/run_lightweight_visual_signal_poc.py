"""Run Phase 1.3-E lightweight visual signal POC on one annotated dataset."""
...
from evaluation.reports.lightweight_visual_signal import (
    ...
    default_lightweight_signal_profiles,
    write_lightweight_signal_report_json,
    write_lightweight_signal_report_markdown,
)

DEFAULT_OUTPUT_ROOT = "outputs/lightweight_visual_signal_poc"


def run_lightweight_visual_signal_poc(
    ...
    grid_shape = SignalGridShape(columns=grid_columns, rows=grid_rows)
    frame_scores = _compute_frame_signal_scores(dataset_config, grid_shape)
    ...
    selected_run_id = run_id or f"{name}_lightweight_visual_signal_{suffix}"
    run_root = Path(output_root) / selected_run_id
    report_root = run_root / "reports"
    frame_records_path = run_root / "lightweight_visual_signal_frames.jsonl"
    report_json_path = report_root / "lightweight_visual_signal_report.json"
    report_md_path = report_root / "lightweight_visual_signal_report.md"

    report, frame_records = build_lightweight_signal_report(
        frame_scores=frame_scores,
        ground_truth=annotations,
        profiles=default_lightweight_signal_profiles(),
    ...
    write_lightweight_signal_report_json(report, report_json_path)
    write_lightweight_signal_report_markdown(report, report_md_path)

    manifest = {
        "schema_version": 1,
        "pipeline_type": "lightweight_visual_signal_poc",
        "experiment_name": name,
...
def _compute_frame_signal_scores(dataset_config, grid_shape: SignalGridShape) -> list[FrameSignalScores]:
    cv2 = _require_cv2()
    records: list[FrameSignalScores] = []
...
def _all_cells(grid_shape: SignalGridShape) -> list[tuple[int, int]]:
    return [
        (row, column)
...
def _require_cv2():
    try:
        ...
        raise ModuleNotFoundError(
            "OpenCV is required for lightweight visual signal POC. Install project dependencies with "
...
def _require_numpy():
    try:
        ...
        raise ModuleNotFoundError(
            "NumPy is required for lightweight visual signal POC. Install project dependencies with "
            "`pip install -r requirements.txt`."
        ) from exc
    return np
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Phase 1.3-E lightweight visual signal POC.")
...
def main() -> None:
    args = parse_args()
    manifest = run_lightweight_visual_signal_poc(

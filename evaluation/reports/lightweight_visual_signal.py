"""Lightweight visual signal POC metrics."""
...
@dataclass(frozen=True)
class LightweightSignalProfile:
    name: str
    signal_name: str
...
@dataclass(frozen=True)
class LightweightSignalFrameRecord:
    camera_id: str
    frame_id: int
    frame_index: int
    profile_name: str
...
@dataclass(frozen=True)
class LightweightSignalProfileReport:
...
class LightweightSignalReport:
    ...
    target_gt_count: int
    target_frame_count: int
    grid_shape: SignalGridShape
    profiles: dict[str, LightweightSignalProfileReport] = field(default_factory=dict)
    ...
    def to_markdown(self) -> str:
        lines = [
            "# Lightweight Visual Signal POC Report",
            "",
            "## Scope",
            "",
            ...
            "",
            (
                "| Profile | Signal | Selected area | Area reduction | Center GT recall | "
...
def default_lightweight_signal_profiles() -> list[LightweightSignalProfile]:
    return [
        LightweightSignalProfile(name="edge_top_10", signal_name="edge", max_area_ratio=0.10),
        LightweightSignalProfile(name="edge_top_20", signal_name="edge", max_area_ratio=0.20),
        LightweightSignalProfile(name="texture_top_10", signal_name="texture", max_area_ratio=0.10),
        LightweightSignalProfile(name="texture_top_20", signal_name="texture", max_area_ratio=0.20),
        LightweightSignalProfile(name="hybrid_top_10", signal_name="hybrid", max_area_ratio=0.10),
        LightweightSignalProfile(name="hybrid_top_20", signal_name="hybrid", max_area_ratio=0.20),
        LightweightSignalProfile(
            name="hybrid_top_20_dilate1",
            signal_name="hybrid",
            max_area_ratio=0.20,
            dilation_cells=1,
        ),
...
def build_lightweight_signal_report(
    frame_scores: Iterable[FrameSignalScores],
    ground_truth: Iterable[GroundTruthAnnotation],
    profiles: Iterable[LightweightSignalProfile],
    grid_shape: SignalGridShape,
    dataset_config: str,
    experiment_name: str,
    target_classes: Iterable[str] | None = None,
) -> tuple[LightweightSignalReport, list[LightweightSignalFrameRecord]]:
    frame_records = list(frame_scores)
    gt_records = list(ground_truth)
    gt_by_frame = group_by_frame(gt_records)
    profile_reports: dict[str, LightweightSignalProfileReport] = {}
    all_records: list[LightweightSignalFrameRecord] = []
    for profile in profiles:
        report, records = evaluate_lightweight_signal_profile(
            frame_scores=frame_records,
...
def evaluate_lightweight_signal_profile(
    frame_scores: list[FrameSignalScores],
    gt_by_frame: dict[tuple[str, int], list[GroundTruthAnnotation]],
    profile: LightweightSignalProfile,
    grid_shape: SignalGridShape,
) -> tuple[LightweightSignalProfileReport, list[LightweightSignalFrameRecord]]:
    records: list[LightweightSignalFrameRecord] = []
    ...
    for frame in frame_scores:
        selected_cells = _select_cells_for_profile(frame, profile, grid_shape)
        ...
        frame_bbox_count = 0
        if gt_records:
            target_frames.add((frame.camera_id, frame.frame_id))
        for gt in gt_records:
            ...
            if _center_cell(gt.bbox_xyxy, frame.frame_size, grid_shape) in selected_cells:
                ...
                center_frames.add((frame.camera_id, frame.frame_id))
            ...
            if bbox_cells and bbox_cells.issubset(selected_cells):
                ...
                frame_bbox_count += 1
                bbox_frames.add((frame.camera_id, frame.frame_id))
        records.append(
            LightweightSignalFrameRecord(
    ...
    report = LightweightSignalProfileReport(
        profile=profile,
...
def write_lightweight_signal_report_json(report: LightweightSignalReport, output_path: str | Path) -> None:
    write_json(report.to_json_dict(), output_path)


def write_lightweight_signal_report_markdown(report: LightweightSignalReport, output_path: str | Path) -> None:
    write_text(report.to_markdown(), output_path)


def _select_cells_for_profile(
    frame: FrameSignalScores,
    profile: LightweightSignalProfile,
    grid_shape: SignalGridShape,
) -> set[tuple[int, int]]:
    try:
    ...
    selected = {_index_to_cell(index, grid_shape) for index, _ in ranked[:base_count]}
...
def _index_to_cell(index: int, grid_shape: SignalGridShape) -> tuple[int, int]:
...
def _cell_in_grid(cell: tuple[int, int], grid_shape: SignalGridShape) -> bool:
    row, column = cell
